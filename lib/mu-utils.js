...
const OCTANE_IMPORT_SPLITTER = "--- hbs ---";
...
function normalizeRightImportPart(right) {
  let importSt = right
    ...
    .split(" ")
    .join("/");
  return normalizeImportTail(importSt);
}
...
function normalizeImportTail(maybeImport) {
  if (maybeImport.startsWith("src/ui/")) {
    // src/ui -> /src/ui
...
function getLetedComponentName(importPath, relativePath) {
  if (importPath.includes("/src/ui")) {
    if (importPath.includes("/-components/")) {
      return `(component (register-absolute-template-import "${relativePath}" "${importPath}"))`;
    } else {
...
module.exports.getLetedComponentName = getLetedComponentName;
